"""
AI Credit Advisor Chatbot - OpenRouter API
Uses cloud-based LLM API for intelligent financial responses
...
- Use plain text only with bullet points (• or -)
...
async def generate_response_async(user_message: str, conversation_history: Optional[List[Dict[str, str]]] = None) -> str:
    """Generate response using OpenRouter API (async version)"""
    try:
        # Build conversation
...
def generate_response(user_message: str, conversation_history: Optional[List[Dict[str, str]]] = None) -> str:
    """Generate response using OpenRouter API (sync wrapper)"""
    import asyncio
    ...
    try:
        ...
        if loop.is_running():
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(
                    asyncio.run,
                    generate_response_async(user_message, conversation_history)
                )
        ...
        return asyncio.run(generate_response_async(user_message, conversation_history))
    except Exception as e:
        print(f"[Chatbot] Sync wrapper error: {e}")
        return fallback_response(user_message)
